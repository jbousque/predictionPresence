...
def prepareMatrix():
    ...
    logger.debug('prepareMatrix: featureMat shape %s', featureMat.shape)
    logger.debug('prepareMatrix: classes shape %s', classes.shape)
...
def preprocess_agent_data():
    ...
    for files in outer_arr:
        if len(files) == 2:
            ...
            np.savetxt(os.path.join(output_dir, 'newtexts.txt'), texts, fmt='%s')
            ...
            newsound.export(os.path.join(output_dir, 'newsound.wav'), format='wav')

            # generate CSV file
            csvfile = open(os.path.join(output_dir, 'newtranscription.csv'), 'wb')
